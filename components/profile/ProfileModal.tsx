...
//   return (
//     <>
//       {/* Backdrop — truly fixed, separate from modal */}
//       <div
//         className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[9998]"
//         onClick={onClose}
//       />

//       {/* Scroll container — above backdrop, pointer-events off so clicks pass to backdrop */}
//       <div className="fixed inset-0 z-[9999] overflow-y-auto pointer-events-none">
//         <div className="flex min-h-full items-center justify-center p-4">

//           {/* Modal card — re-enable pointer events */}
//           <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md pointer-events-auto">

//             {/* Nudge banner */}
//             {showNudge && (
//               <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-5 py-2.5
//                               border-b border-blue-100 rounded-t-3xl">
...
export default function ProfileModal({ onClose, showNudge = false }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(false)
    setSaved(true)
    setTimeout(() => { setSaved(false); onClose(); router.refresh() }, 1000)
  }
  ...
  const handleDeleteConfirmed = async () => {
    ...
    if (authError) { setDeleteError('Incorrect password.'); setDeleting(false); return }
    await fetch('/api/profile', { method: 'DELETE' })
    router.push('/login')
  }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          pointerEvents: 'auto', maxHeight: '90vh',
          ...
          <div className="px-6 pt-5 pb-3 flex items-center justify-between flex-shrink-0">
            ...
            <button onClick={onClose}
              className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200
                         flex items-center justify-center text-slate-500 text-sm transition-colors">
              ×
            </button>
          </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="flex items-center gap-4">
                  <button onClick={() => fileRef.current?.click()}
                    className="relative group w-16 h-16 rounded-full flex-shrink-0 overflow-hidden
                               bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center">
                    {avatar
                      ? <img src={avatar} alt="avatar" className="w-full h-full object-cover" />
                      : <span className="text-xl font-light text-blue-400">{initials}</span>
                    }
                  ...
                  <div>
                    <p className="text-sm text-slate-600 font-light">{profile.name || 'No name yet'}</p>
                    <button onClick={() => fileRef.current?.click()}
                      className="text-xs text-blue-400 hover:text-blue-500 mt-0.5">
                      Change photo
                    </button>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 rounded-xl p-4 space-y-1">
                  ...
                  <p className="text-xs text-slate-400">{email}</p>
                  {profile.bio && (
                    <p className="text-xs text-slate-400 pt-1 border-t border-slate-100 mt-1">{profile.bio}</p>
                  )}
                </div>
                ...
                <div className="border border-red-100 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 bg-red-50">
                    <p className="text-xs font-medium text-red-400">Delete account</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">
                      Permanently deletes all entries, transactions, reports and your profile. Cannot be undone.
                    </p>
                  </div>
                  <div className="p-4 space-y-3">
                    {deleteStep === 'idle' && (
                    ...
                    {deleteStep === 'confirm' && (
                      <div className="space-y-3">
                        <p className="text-xs text-slate-500 text-center">This will permanently delete:</p>
                        <ul className="text-xs text-slate-400 space-y-1 pl-3">
                          <li>• All journal entries</li>
                          <li>• All transactions & receipts</li>
                          <li>• All weekly reports</li>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <button onClick={() => setDeleteStep('password')}
                            className="py-2.5 bg-red-400 text-white rounded-xl text-xs hover:bg-red-500 transition-colors">
                            Yes, continue
                          </button>
                        </div>
                      </div>
                    ...
                    {deleteStep === 'password' && (
                      <div className="space-y-3">
                        <p className="text-xs text-slate-500 text-center">Enter your password to confirm</p>
